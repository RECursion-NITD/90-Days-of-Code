...
tree query_old(int treeIndex,int treeLeft,int treeRight,int l,int r){
    ...
    if(treeMid<l)return query(2*treeIndex+1,treeMid+1,treeRight,l,r);
    if(treeMid>=r)return query(2*treeIndex,treeLeft,treeMid,l,r);
    return join(query(2*treeIndex,treeLeft,treeMid,l,r),query(2*treeIndex+1,treeMid+1,treeRight,l,r));
}
